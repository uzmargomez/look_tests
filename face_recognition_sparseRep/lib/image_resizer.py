...
def resize_images(MAX_DIM=400, DATASET="datasets/**", EXT="jpg", VIS=False):
    ...
    for i_dir, dir in enumerate(all_dirs):
        ...
        for i_path, path in enumerate(all_paths):
            print(
                "Resizing directories {:<3}/{:<3} Resizing images {:<3}/{:<3}".format(
                    i_dir + 1, len(all_dirs), i_path + 1, len(all_paths)
                ),
                end="\r",
            )
